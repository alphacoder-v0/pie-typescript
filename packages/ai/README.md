...
TypeBox exports are re-exported from `@pie/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@pie/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@pie/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@pie/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@pie/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@pie/ai';
...
```typescript
import { getModel, stream } from '@pie/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@pie/ai';
...
```typescript
import { Model, stream } from '@pie/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@pie/ai';
...
```typescript
import { getModel, complete, Context } from '@pie/ai';
...
```typescript
import { Context, getModel, complete } from '@pie/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@pie/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@pie/ai';
...
npx @pie/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@pie/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@pie/ai/oauth';
...
```typescript
import { getModel, complete } from '@pie/ai';
import { getOAuthApiKey } from '@pie/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@pie/ai`
