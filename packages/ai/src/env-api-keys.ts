...
function getApiKeyEnvVars(provider: string): readonly string[] | undefined {
	...
	// pie: crates/ai/src/env_api_keys.rs:11 GOOGLE_API_KEY takes precedence over GEMINI_API_KEY
	if (provider === "google") {
		return ["GOOGLE_API_KEY", "GEMINI_API_KEY"];
	}

	// pie: crates/ai/src/env_api_keys.rs:26 HUGGINGFACE_API_KEY takes precedence over HF_TOKEN
	...
	const envMap: Record<string, string> = {
		...
		// pie: crates/ai/src/env_api_keys.rs:9 openai-codex falls back to OPENAI_API_KEY like openai
		...
		// pie: crates/ai/src/env_api_keys.rs:22 ds4 (local DeepSeek V4 server) dedicated key
		ds4: "DS4_API_KEY",
	};
...
/**
 * List the environment variable names that could provide an API key for a provider,
 * regardless of whether they are currently set. Used for diagnostics/help text
 * (e.g. "set one of X, Y to configure this provider").
 *
 * pie: crates/ai/src/env_api_keys.rs:45 (`env_var_names`) — 1:1 port of the candidate-name
 * lookup (unfiltered by whether the variable is actually present in the environment).
 */
export function envVarNames(provider: KnownProvider): readonly string[];
export function envVarNames(provider: string): readonly string[];
export function envVarNames(provider: string): readonly string[] {
	return getApiKeyEnvVars(provider) ?? [];
}
