...
export interface OpenAIResponsesCompat {
	...
	supportsLongCacheRetention?: boolean;
	/**
	 * Whether replayed assistant thinking content must be re-sent as a `{"type":"reasoning"}`
	 * input item (with the raw thinking text) even when no OpenAI-issued reasoning signature is
	 * available. Needed by servers that do byte-exact KV prefix matching on the rendered history
	 * (e.g. ds4 / DeepSeek V4 local): omitting the reasoning item changes the rendered prefix and
	 * invalidates their cache checkpoints. Default: false.
	 */
	requiresReasoningContentOnAssistantMessages?: boolean;
}
