...
export const streamOpenAICodexResponses: StreamFunction<"openai-codex-responses", OpenAICodexResponsesOptions> = (
	...
	(async () => {
		...
		try {
			...
			// pie: crates/ai/src/providers/openai_codex_responses.rs:157 (run) +
			// crates/ai/src/utils/retry.rs — oracle's codex provider retries exclusively through
			// `crate::utils::retry::send_with_retry`, the same status/transport-driven util every other
			// provider uses. The previous hand-rolled loop here had three pi-only extensions with no
			// oracle counterpart: a nonstandard `retry-after-ms` response header, HTTP-date parsing for
			// `retry-after` (retry.ts intentionally doesn't support this either, matching oracle), and
			// suppressing retries when the error text contained "usage limit". None of those exist in
			// oracle's `send_with_retry` (status/network-only, no response-body text sniffing), so this
			// now wires straight through `sendWithRetry` like the other Responses-family providers.
...
function buildRequestBody(
	...
	const body: RequestBody = {
		...
		// pie: crates/ai/src/providers/openai_codex_responses.rs:202-204 (build_request_body) —
		// oracle sets `prompt_cache_key` to the raw session id with no length clamp anywhere in the
		// ai crate (confirmed: no `chars().take(64)`/truncation exists in the whole crate; see also
		// crates/ai/src/providers/openai_responses.rs:591, which is equally unclamped). The
		// `clampOpenAIPromptCacheKey` 64-char guard is a pi-only safety net that this call site
		// bypasses to match oracle bug-for-bug; openai-responses.ts (pilot-locked) keeps the clamp.
	...
	if (context.tools && context.tools.length > 0) {
		// pie: crates/ai/src/providers/openai_responses.rs:648-660 (serialize_tools, reused directly
		// by openai_codex_responses.rs via `use crate::providers::openai_responses::{..., serialize_tools}`)
		// — oracle never emits a "strict" key on tool items.
		body.tools = convertResponsesTools(context.tools, { omitStrict: true });
	}
...
function buildBaseCodexHeaders(
	...
	// pie: crates/ai/src/providers/openai_codex_responses.rs:140-152 (run) — oracle's codex header set
	// has no explicit User-Agent call; the value comes entirely from
	// crates/ai/src/utils/node_http_proxy.rs:19-22 (build_client)'s HTTP client default
	// ("pie-ai-rs/<version>"), same as every other provider. The pi-specific
	// "pi (${platform} ${release}; ${arch})" identity string previously set here has no oracle
	// counterpart — replaced with the shared default so codex matches the other 8 providers.
