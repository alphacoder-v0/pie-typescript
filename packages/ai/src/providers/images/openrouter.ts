...
function parseUsage(
	...
	// This provider already priced its own usage: it is pi-only (oracle has no images crate), so it
	// was never under B3a's bug-for-bug hold. Routed through the shared `computeCost` so the last
	// remaining copy of the per-million pricing arithmetic cannot drift from the LLM providers'.
	// `input` above is already net of both cache buckets, as `computeCost` requires.
	const tokens = {
		input,
		output,
