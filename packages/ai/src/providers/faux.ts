...
export function fauxAssistantMessage(
	...
	const normalizedContent = normalizeFauxAssistantContent(content);
	return {
		role: "assistant",
		...
		usage: DEFAULT_USAGE,
		// pie: crates/ai/src/providers/faux.rs:71-93 (faux_assistant_message) — oracle infers
		// stopReason "toolUse" when content contains a tool-call block and the caller didn't pass
		// an explicit stopReason, instead of unconditionally defaulting to "stop".
		stopReason:
			options.stopReason ?? (normalizedContent.some((block) => block.type === "toolCall") ? "toolUse" : "stop"),
...
function withUsageEstimate(
	...
	return {
		...message,
		usage: {
			// Every prompt token is billed to exactly one bucket: the cached prefix to `cacheRead`,
			// the freshly-written remainder to `cacheWrite`, and only what neither covers (normally
			// nothing, since those two partition the prompt) to `input`. That is the shape real
			// providers report -- Anthropic's and Bedrock's input counts EXCLUDE the cache buckets,
			// which is precisely why `totalTokens` is the sum of all four. Until phase 19 this
			// function kept `input` at the full `promptTokens` across a cache write, so a fresh
			// cached turn reported `2 x prompt + output` and a warm one counted the non-cached
			// suffix under both `input` and `cacheWrite`. The totals now come from `finalizeUsage`
			// -- the one owner of this arithmetic (see src/usage.ts) -- instead of a local copy of
			// the formula, which is how the two drifted apart in the first place. It also clamps,
			// so the +/-1 slack in `estimateTokens`'s per-slice ceil cannot produce a negative
			// `input`.
			...finalizeUsage({
				uncachedInput: promptTokens - cacheRead - cacheWrite,
				output: outputTokens,
				cacheRead,
				cacheWrite,
			}),
