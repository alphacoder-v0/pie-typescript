...
import { DEFAULT_MAX_RETRIES } from "../utils/retry.ts";
...
export const streamBedrock: StreamFunction<"bedrock-converse-stream", BedrockOptions> = (
	...
	(async () => {
		...
		// TODO(port): AWS SDK v3 fetch injection unavailable — attempts aligned to oracle retry budget
		// instead. pie: crates/ai/src/providers/amazon_bedrock.rs:107 + crates/ai/src/utils/retry.rs —
		// oracle sends every attempt through `send_with_retry` (its own retryable-status set and
		// backoff/cap semantics), same as anthropic.ts/openai-completions.ts/openai-responses.ts, which
		// all wire `sendWithRetry` into the SDK's underlying `fetch`. This SDK version's transport
		// handlers have no such hook: `FetchHttpHandler` (@smithy/fetch-http-handler) calls the global
		// `fetch` directly with no config-level fetch override, and the Node-side handlers
		// (`NodeHttpHandler`/`NodeHttp2Handler`, used by default per the User-Agent comment below) are
		// raw http(s)/http2 clients with no fetch involved at all — there is nothing to wrap with
		// `sendWithRetry`. Falling back to the SDK's own built-in retry, with its attempt count aligned
		// to oracle's default retry budget (retry.rs:17, `DEFAULT_MAX_RETRIES` = 2 retries beyond the
		// first attempt = 3 total attempts) so at least the retry *count* matches, even though the
		// retryable-status set, backoff curve, and Retry-After handling remain the AWS SDK's own.
		...
		try {
			...
			// pie-ai-rs/<version> as the HTTP client default for every outbound request. The AWS SDK
			// v3 userAgentMiddleware (step "build") unconditionally overwrites the User-Agent header
			// with its own "aws-sdk-js/... ua/2.1 os/... lang/js md/... api/bedrock-runtime#..."
			// string; `customUserAgent` config only appends segments and cannot replace the header
			// outright. Add a finalizeRequest-step middleware (runs after "build") to force the
			// literal oracle value after the SDK's own middleware has already run. Guarded on
			// `middlewareStack` existing: unit tests mock `BedrockRuntimeClient` with a bare
			// {constructor, send} stub that has no middleware stack at all.
			client.middlewareStack?.add?.(
				(next) => async (args) => {
					if (HttpRequest.isInstance(args.request)) {
						// lowercase "user-agent": the SDK's own userAgentMiddleware (build step) writes the
						// key lowercase (@aws-sdk/core USER_AGENT constant); a differently-cased key here
						// would leave both on the HeaderBag and Node's http client rejects duplicate
						// case-variant header names ("must only have a single value").
						args.request.headers["user-agent"] = userAgent();
					}
					return next(args);
				},
				{ step: "finalizeRequest", name: "pieUserAgentOverride", tags: ["USER_AGENT"] },
			);
...
function handleMetadata(
	...
	if (event.usage) {
		...
		// pie: crates/ai/src/providers/amazon_bedrock.rs:348-360 (update_usage) — oracle recomputes
		// totalTokens as input+output+cacheRead+cacheWrite instead of trusting the SDK-provided
		// TokenUsage.totalTokens. That recomputation AGREES with AWS rather than diverging from it:
		// ConverseStream reports `inputTokens` EXCLUSIVE of the cache buckets, so AWS's own total is
		// the same four-way sum — its prompt-caching example is inputTokens 4 + outputTokens 106 +
		// cacheWriteInputTokens 2349 = totalTokens 2459. Nothing is double-counted here.
		// (An earlier revision of this comment asserted AWS defines totalTokens as input+output only
		// and therefore that this line double-counts cache tokens. Both halves were false.)
		// Recomputing is still the right call rather than merely redundant: it keeps totalTokens
		// consistent with the four fields set immediately above no matter what the payload claims,
		// so a partial or internally inconsistent metadata event cannot smuggle in a contradictory
		// total. Do not prefer event.usage.totalTokens.
		output.usage.totalTokens =
			output.usage.input + output.usage.output + output.usage.cacheRead + output.usage.cacheWrite;
		// PORT-DIVERGENCE: B3a (RULEBOOK §5) — crates/ai/src/providers/amazon_bedrock.rs:348-360's
		// update_usage never touches Usage.cost; independently confirmed no provider file in oracle
		// calls a cost-calculation helper, which is what pinned every user-visible cost figure at $0
		// (B3). Phase 18 prices it here. AWS's ConverseStream reports `inputTokens` EXCLUSIVE of
		// `cacheReadInputTokens`/`cacheWriteInputTokens`, so it is already the uncached input
		// `computeCost` requires — and that same exclusivity is what makes the four-way sum above
		// the correct total rather than a double count.
...
function mapStopReason(reason: string | undefined): StopReason {
	switch (reason) {
		...
		case BedrockStopReason.TOOL_USE:
			return "toolUse";
		default:
			// pie: crates/ai/src/providers/amazon_bedrock.rs:339-346 (map_stop_reason) — oracle's
			// catch-all arm maps any unrecognized stop reason string to Stop, not an error state.
			// Bug-for-bug: only the true default fallback changes here; the MODEL_CONTEXT_WINDOW_EXCEEDED
			// -> "length" case above has no counterpart in oracle's minimal 4-arm match (oracle would
			// also fall through to Stop for it) but is kept as-is — it drives real overflow-detection
			// behavior (see utils/overflow.ts) that oracle's admittedly partial Bedrock port never
			// reaches; stripping it would be a product regression with no RULEBOOK authorization.
			return "stop";
	}
}
