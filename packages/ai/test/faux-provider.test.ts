...
describe("faux provider", () => {
	...
	it("unregisters the provider", async () => {
		...
		// pie: crates/ai/src/stream.rs:13-17,19-28 (resolve + stream) — oracle encodes "no provider
		// registered" as an error-stream event, so complete()'s promise resolves (not rejects) with
		// an AssistantMessage carrying stopReason "error".
		const result = await complete(registration.getModel(), {
			messages: [{ role: "user", content: "hi", timestamp: Date.now() }],
		});
		expect(result.stopReason).toBe("error");
		expect(result.errorMessage).toBe(`No API provider registered for api: ${registration.api}`);
	});
});

describe("fauxAssistantMessage stopReason inference (bug-for-bug: oracle faux.rs faux_assistant_message)", () => {
	it("infers toolUse when content contains a tool call and stopReason is omitted", () => {
		const message = fauxAssistantMessage([fauxToolCall("echo", { text: "hi" })]);
		expect(message.stopReason).toBe("toolUse");
	});

	it("still defaults to stop when content has no tool call", () => {
		const message = fauxAssistantMessage([fauxText("hi")]);
		expect(message.stopReason).toBe("stop");
	});

	it("respects an explicit stopReason override even with a tool call present", () => {
		const message = fauxAssistantMessage([fauxToolCall("echo", {})], { stopReason: "error" });
