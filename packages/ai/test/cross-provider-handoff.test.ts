...
/**
 * A *handoff* needs two providers, not one.
 *
 * The fixture-count assertion below used to be gated on {@link hasAnyApiKey}, so with exactly one
 * provider credentialed it ran and failed on `expected 1 to be greater than or equal to 2` — an
 * expectation the environment could not satisfy, yet it was not skipped. This stayed invisible for
 * as long as the hermetic suite unset every key (the whole describe skipped), and surfaced the
 * first time the suite ran against a real provider with a single credential available.
 */
function hasAtLeastTwoApiKeys(): boolean {
	return PROVIDER_MODEL_PAIRS.filter((pair) => hasApiKey(pair)).length >= 2;
}
...
describe.skipIf(!hasAnyApiKey())("Cross-Provider Handoff", () => {
	...
	it.skipIf(!hasAtLeastTwoApiKeys())("should have at least 2 fixtures to test handoffs", () => {
