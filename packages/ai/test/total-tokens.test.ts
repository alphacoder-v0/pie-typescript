...
describe("totalTokens field", () => {
	...
	describe.skipIf(!process.env.GEMINI_API_KEY)("Google", () => {
		it(
			...
			{ retry: 3, timeout: 60000 },
			async () => {
				// gemini-2.0-flash was retired by Google — the API 404s with "no longer available".
				// Switched to gemini-2.5-flash, which the other live google tests here already exercise.
				// The catalog still advertises the retired id; that is oracle's frozen snapshot, pinned
				// byte-for-byte by parity S1, so it is recorded in the backlog rather than edited.
