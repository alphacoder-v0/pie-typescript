...
describe("openai-completions tool_choice", () => {
	...
	it("preserves prompt_tokens_details cache read/write fields from chunk usage", async () => {
		...
		// pie: crates/ai/src/providers/openai_completions.rs:442-456 (update_usage) —
		// PORT-DIVERGENCE: B2 (RULEBOOK §5). Oracle kept `input` at the raw prompt_tokens value
		// (which already includes cached_tokens) and summed
		// totalTokens = input + output + cacheRead + cacheWrite, reporting 185 for these numbers.
		// Phase 18 nets both cache buckets out of input first: 100 - 50 - 30 = 20 uncached input,
		// so the cached tokens are counted exactly once and the total is 20 + 50 + 30 + 5 = 105.
