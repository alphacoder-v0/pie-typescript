...
describe("openai-completions thinking-as-text replay", () => {
	...
	it("reaches the endpoint when replay contains both thinking and text", async () => {
		...
		const server = http.createServer(async (req, res) => {
			// pie: crates/ai/src/providers/openai_completions.rs:462-470 (normalize_base) — oracle
			// normalizes a bare-host baseUrl to include "/v1" before "/chat/completions"; the base
			// client now does the same (see normalizeOpenAICompletionsBaseUrl), so a bare-host baseUrl
			// hits "/v1/chat/completions", not "/chat/completions".
