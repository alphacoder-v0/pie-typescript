...
describe("openai-codex streaming", () => {
	...
	// pie: crates/ai/src/providers/openai_codex_responses.rs:202-204 (build_request_body) — oracle
	// sends the raw session id as `prompt_cache_key` with no length clamp anywhere in the ai crate
	// (grepped whole crate for a 64-char truncation helper: none exists). Previously this test
	// asserted a pi-only 64-char clamp that has no oracle counterpart; it now locks the oracle-faithful
	// unclamped passthrough instead.
	it("does not clamp prompt_cache_key (oracle sends the raw session id, no length limit)", async () => {
		...
		expect(capturedPayload?.prompt_cache_key).toHaveLength(67);
	});
	...
	// pie: crates/ai/src/providers/openai_codex_responses.rs:157 (run) + crates/ai/src/utils/retry.rs
	// — codex now retries through the same `sendWithRetry` util as every other provider (oracle's
	// `send_with_retry` is status/transport-driven only, honoring only the standard `Retry-After`
	// header, seconds form). The nonstandard `retry-after-ms` header and HTTP-date `Retry-After`
	// parsing were pi-only extensions with no oracle counterpart (retry.ts intentionally doesn't
	// support HTTP-date either, matching oracle) — both now fall back to computed backoff
	// (500ms * 2^attempt + 1-100ms jitter, i.e. [501, 600] on the first attempt) instead of honoring
	// the header value. `retry-after: <seconds>` is still honored exactly (no jitter applied when a
	// server delay is present).
	it.each([
		[
			"retry-after-ms (nonstandard, ignored)",
			() => ({ "content-type": "application/json", "retry-after-ms": "1500" }),
			[501, 600],
		],
		["retry-after seconds", () => ({ "content-type": "application/json", "retry-after": "60" }), [60_000, 60_000]],
		[
			"retry-after HTTP date (unsupported, ignored)",
			() => ({ "content-type": "application/json", "retry-after": new Date(Date.now() + 45_000).toUTCString() }),
			[501, 600],
		],
	] as const)("uses %s for SSE retries", async (_name, makeHeaders, [minDelay, maxDelay]) => {
		vi.useFakeTimers();
		...
		const [, actualDelay] = setTimeoutSpy.mock.calls[0]!;
		expect(actualDelay as number).toBeGreaterThanOrEqual(minDelay);
		expect(actualDelay as number).toBeLessThanOrEqual(maxDelay);
	...
	// pie: crates/ai/src/utils/retry.rs:160-168 (backoff_delay) — sendWithRetry's computed backoff is
	// `500ms * 2^attempt + [1,100]ms jitter` (retry.ts DEFAULT_BASE_DELAY_MS=500), not codex's old
	// 1000ms-base/no-jitter formula. Ranges: attempt0 [501,600], attempt1 [1001,1100]. Also, oracle's
	// default max_retries is 2 (retry.rs:17), not codex's old hardcoded 3 — 2 failures then success.
	it("uses exponential backoff across repeated SSE retries without retry headers", async () => {
		...
		const fetchMock = vi.fn(async (input: string | URL) => {
			...
			codexRequests++;
			// pie: crates/ai/src/utils/retry.rs:17-19 — oracle's default max_retries is 2 (3 total
			// attempts), not the old codex-specific hardcoded 3 (4 total attempts).
			if (codexRequests <= 2) {
				return new Response(JSON.stringify({ error: { code: "rate_limit_exceeded", message: "rate limited" } }), {
					status: 429,
		...
		const resultPromise = streamOpenAICodexResponses(model, context, { apiKey: token, transport: "sse" }).result();
		await vi.advanceTimersByTimeAsync(0);
		const [, delay1] = setTimeoutSpy.mock.calls[0]!;
		expect(delay1 as number).toBeGreaterThanOrEqual(501);
		expect(delay1 as number).toBeLessThanOrEqual(600);

		await vi.advanceTimersToNextTimerAsync();
		const [, delay2] = setTimeoutSpy.mock.calls[1]!;
		expect(delay2 as number).toBeGreaterThanOrEqual(1001);
		expect(delay2 as number).toBeLessThanOrEqual(1100);

		await vi.advanceTimersToNextTimerAsync();
		const result = await resultPromise;
		expect(result.content.find((content) => content.type === "text")?.text).toBe("Hello");
		expect(codexRequests).toBe(3);
	});
});
