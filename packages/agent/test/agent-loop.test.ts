...
describe("agentLoop with AgentMessage", () => {
	...
	// pie: agent_loop.rs:399-421,867-953 (issue #110 design v0.2 Artifact A) — a tool's
	// permissionClassification returning "block" short-circuits before beforeToolCall and
	// before execute, with no control-plane prompt at all.
	it("blocks a tool call hard when permissionClassification returns block, without running execute", async () => {
		const toolSchema = Type.Object({ value: Type.String() });
		let executed = false;
		...
		const tool: AgentTool<typeof toolSchema, { value: string }> = {
			...
			parameters: toolSchema,
			permissionClassification: () => ({ type: "block", reason: "never allowed" }),
			async execute(_toolCallId, params) {
				executed = true;
		...
		const stream = agentLoop([createUserMessage("go")], context, config, undefined, () => {
			...
			queueMicrotask(() => {
				if (llmCalls === 1) {
					const message = createAssistantMessage(
						[{ type: "toolCall", id: "tool-1", name: "danger", arguments: { value: "x" } }],
						"toolUse",
					);
		...
		expect(events.some((e) => e.type === "control_plane_prompt_resolved")).toBe(false);
		const toolEnd = events.find((e) => e.type === "tool_execution_end");
		expect(toolEnd).toMatchObject({ isError: true, result: { content: [{ type: "text", text: "never allowed" }] } });
	});

	// pie: agent_loop.rs:521-587 — with no onControlPlanePrompt hook configured, a "prompt"
	// classification fails closed to deny (never silently allows an escalating write).
	it("fails closed to deny when permissionClassification requests a prompt but no onControlPlanePrompt hook is configured", async () => {
		const toolSchema = Type.Object({ value: Type.String() });
		let executed = false;
		const tool: AgentTool<typeof toolSchema, { value: string }> = {
			name: "install",
			...
			parameters: toolSchema,
			permissionClassification: () => ({ type: "prompt", reason: "installs a skill" }),
			async execute(_toolCallId, params) {
				executed = true;
				return { content: [{ type: "text", text: params.value }], details: { value: params.value } };
			},
		...
		const stream = agentLoop([createUserMessage("go")], context, config, undefined, () => {
			...
			queueMicrotask(() => {
				if (llmCalls === 1) {
					const message = createAssistantMessage(
						[{ type: "toolCall", id: "tool-1", name: "install", arguments: { value: "x" } }],
						"toolUse",
					);
					mockStream.push({ type: "done", reason: "toolUse", message });
				} else {
		...
		expect(llmCalls).toBe(2);
		expect(executed).toBe(false);
		const resolved = events.find((e) => e.type === "control_plane_prompt_resolved");
		expect(resolved).toMatchObject({ toolCallId: "tool-1", toolName: "install", decision: "deny" });
		const toolEnd = events.find((e) => e.type === "tool_execution_end");
		expect(toolEnd).toMatchObject({ isError: true });
	});

	// pie: agent_loop.rs:521-587 — an "allow" decision from onControlPlanePrompt dispatches
	// the tool normally, and the resolved event carries the redaction-safe default payload
	// (tool_name/args_keys/args_hash only, never raw args).
	it("dispatches the tool after onControlPlanePrompt resolves allow, with a redaction-safe default payload", async () => {
		const toolSchema = Type.Object({ value: Type.String(), secret: Type.String() });
		let executed = false;
		const tool: AgentTool<typeof toolSchema, { value: string }> = {
			name: "install",
			label: "Install",
		...
		const config: AgentLoopConfig = {
			model: createModel(),
			convertToLlm: identityConverter,
			onControlPlanePrompt: async (request) => {
		...
		const stream = agentLoop([createUserMessage("go")], context, config, undefined, () => {
			...
			queueMicrotask(() => {
				if (llmCalls === 1) {
					const message = createAssistantMessage(
						[{ type: "toolCall", id: "tool-1", name: "install", arguments: { value: "x", secret: "sk-shhh" } }],
						"toolUse",
					);
					mockStream.push({ type: "done", reason: "toolUse", message });
				} else {
		...
		const resolved = events.find((e) => e.type === "control_plane_prompt_resolved");
		expect(resolved).toMatchObject({ toolCallId: "tool-1", toolName: "install", decision: "allow" });
		expect(receivedPayload).toMatchObject({ tool_name: "install", args_keys: ["secret", "value"] });
		expect(JSON.stringify(receivedPayload)).not.toContain("sk-shhh");
	});
	...
	// pie: agent_loop.rs:293-298,321 (call_llm's match arm + catch-all `_ => {}`) — TextStart/
	// ThinkingStart/ToolCallStart are not matched by the delta/end arm and fall through to the
	// catch-all, so oracle emits nothing for them (no state update, no event). Only the
	// message-level Start (MessageStart) and the delta/end sub-events produce message_update.
	it("silently drops text_start/thinking_start/toolcall_start sub-events", async () => {
		const context: AgentContext = { systemPrompt: "", messages: [], tools: [] };
		const config: AgentLoopConfig = { model: createModel(), convertToLlm: identityConverter };

		const startPartial = createAssistantMessage([{ type: "text", text: "" }]);
		const deltaPartial = createAssistantMessage([{ type: "text", text: "hi" }]);
		...
		const stream = agentLoop([createUserMessage("hi")], context, config, undefined, () => {
			const mockStream = new MockAssistantStream();
			queueMicrotask(() => {
				mockStream.push({ type: "start", partial: startPartial });
				mockStream.push({ type: "text_start", contentIndex: 0, partial: startPartial });
				mockStream.push({ type: "text_delta", contentIndex: 0, delta: "hi", partial: deltaPartial });
				mockStream.push({ type: "text_end", contentIndex: 0, content: "hi", partial: deltaPartial });
		...
		// text_start must NOT produce a message_update; only text_delta and text_end do.
		expect(updateEvents.map((e) => e.assistantMessageEvent.type)).toEqual(["text_delta", "text_end"]);
	});

	// pie: agent_loop.rs:124-135 — should_stop_after_turn is evaluated before prepare_next_turn,
	// against the pre-prepare_next_turn context/config.
	it("checks shouldStopAfterTurn before prepareNextTurn, using the pre-prepare_next_turn context", async () => {
		const context: AgentContext = { systemPrompt: "original", messages: [], tools: [] };
		...
		const config: AgentLoopConfig = {
			...
			prepareNextTurn: async () => {
				...
				return { context: { systemPrompt: "mutated", messages: [], tools: [] } };
			},
			shouldStopAfterTurn: async ({ context: currentContext }) => {
				observedSystemPrompt = currentContext.systemPrompt ?? "";
				return true;
			},
		};
		...
		expect(observedSystemPrompt).toBe("original");
		expect(prepareNextTurnCalls).toBe(0);
	});

	// pie: agent_loop.rs:137-154 — a tool batch that unanimously requests early termination is
	// an unconditional hard stop: prepare_next_turn does not run and steering/follow-up queues
	// are never drained, so a queued message cannot resurrect a terminated batch.
	it("terminate is a hard stop that skips prepareNextTurn and steering/follow-up queue draining", async () => {
		const toolSchema = Type.Object({ value: Type.String() });
		const tool: AgentTool<typeof toolSchema, { value: string }> = {
			name: "echo",
			label: "Echo",
		...
		const config: AgentLoopConfig = {
			...
			getSteeringMessages: async () => {
				steeringPolls++;
				// First call is the pre-turn-1 poll (nothing queued yet); a second call would
				// only happen if the loop reached the post-turn queue-draining step, which the
				// terminate hard-stop must skip.
				if (steeringPolls === 1) return [];
				return [createUserMessage("steering should not be injected")];
			},
			getFollowUpMessages: async () => {
				followUpPolls++;
				return [createUserMessage("follow-up should not be injected")];
			},
		};
		...
		const messages = await stream.result();
		// The initial getSteeringMessages poll (before turn 1) still fires; the post-turn poll
		// inside the loop body must not, since terminate short-circuits before reaching it.
		expect(steeringPolls).toBe(1);
		expect(followUpPolls).toBe(0);
		expect(prepareNextTurnCalls).toBe(0);
		expect(llmCalls).toBe(1);
	...
	// pie: agent_loop.rs:265-323 (call_llm's biased select) — races the stream's next event
	// against the abort signal so a stalled provider that never emits and never notices `signal`
	// doesn't block the loop indefinitely. Closes oracle issue #18.
	it("aborts a stalled stream promptly via the biased cancellation race (closes oracle issue #18)", async () => {
		const context: AgentContext = { systemPrompt: "", messages: [], tools: [] };
		const config: AgentLoopConfig = { model: createModel(), convertToLlm: identityConverter };
		...
		const runPromise = runAgentLoop(
			...
			controller.signal,
			() => new MockAssistantStream(), // never pushes any event — a stalled provider
		);
...
describe("agentLoopContinue with AgentMessage", () => {
	...
	// pie: agent_loop.rs:55-64 (run_agent_loop_continue) — oracle has no "last message must not
	// be assistant" guard at all, only the empty-transcript check above. A caller trusted to
	// supply a valid continuation point is not defensively blocked here.
	it("no longer throws when the last message is an assistant message", async () => {
		const context: AgentContext = {
			systemPrompt: "You are helpful.",
			messages: [createUserMessage("Hello"), createAssistantMessage([{ type: "text", text: "Hi" }])],
			tools: [],
		};
		...
		const streamFn = () => {
			const stream = new MockAssistantStream();
			queueMicrotask(() => {
				const message = createAssistantMessage([{ type: "text", text: "Again" }]);
				stream.push({ type: "done", reason: "stop", message });
			});
			return stream;
		};
