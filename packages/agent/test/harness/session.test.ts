...
import type { CompactionEntry, SessionStorage } from "../../src/harness/types.ts";
...
async function runSessionSuite(
	...
	describe(name, () => {
		...
		// pie: crates/agent/src/harness/session/session.rs:418-432 — session_name() walks
		// entries newest-first and skips blank names instead of only checking the latest entry.
		it("falls back to an earlier session name when the latest rename is blank", async () => {
			const session = new Session(await createStorage());
			await session.appendSessionName("first name");
			await session.appendSessionName("   ");
			expect(await session.getSessionName()).toBe("first name");
		});

		// pie: crates/agent/src/harness/session/session.rs:502,584 — from_hook is `Some(true)`
		// or `None`; an explicit `false` must serialize identically to "not a hook", never as a
		// literal `false` key.
		it("normalizes fromHook to true-or-undefined on compaction and branch-summary entries", async () => {
			const session = new Session(await createStorage());
			const user1 = await session.appendMessage(createUserMessage("one"));

			await session.appendCompaction("hooked", user1, 10, undefined, true);
			await session.appendCompaction("not hooked", user1, 10, undefined, false);
			...
			expect(compactions[1]?.fromHook).toBeUndefined();

			const hookedSummaryId = await session.moveTo(user1, { summary: "hooked summary", fromHook: true });
			const hookedSummary = await session.getEntry(hookedSummaryId!);
			expect(hookedSummary).toMatchObject({ type: "branch_summary", fromHook: true });

			const plainSummaryId = await session.moveTo(user1, { summary: "plain summary", fromHook: false });
			const plainSummary = await session.getEntry(plainSummaryId!);
			expect(plainSummary).toMatchObject({ type: "branch_summary", fromHook: undefined });
		});
