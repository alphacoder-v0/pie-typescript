...
describe("resource formatting helpers", () => {
	...
	// pie: skills.rs:23-34 `additional_instructions: Option<&str>` -- an explicit `Some("")`
	// still appends the "\n\n" separator; only a fully-absent argument (`undefined`) skips it.
	it("appends the separator for an explicit empty additionalInstructions string", () => {
		const skill = {
			name: "inspect",
			description: "Inspect things",
			content: "Use inspection tools.",
	...
	it("formats prompt template invocations with positional arguments", () => {
		expect(
			formatPromptTemplateInvocation(
				{ name: "review", content: "Review $1 with $ARGUMENTS", filePath: "/templates/review.md" },
				["a.ts", "care"],
			),
