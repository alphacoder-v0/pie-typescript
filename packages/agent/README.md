...
Stateful agent with tool execution and event streaming. Built on `@pie/ai`.

## Installation

```bash
npm install @pie/agent-core
```
...
```typescript
import { Agent, streamProxy } from "@pie/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@pie/agent-core";
