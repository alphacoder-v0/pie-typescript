...
export class Session<TMetadata extends SessionMetadata = SessionMetadata> {
	...
	async getSessionName(): Promise<string | undefined> {
		// pie: crates/agent/src/harness/session/session.rs:418-432 — walk session_info entries
		// newest-first and return the first non-blank trimmed name, not just the latest entry's
		// name. A later blank/whitespace name (e.g. from a cleared rename) must not shadow an
		// earlier real name.
		const entries = await this.storage.findEntries("session_info");
		for (let i = entries.length - 1; i >= 0; i--) {
			const trimmed = entries[i]?.name?.trim();
	...
	async appendCompaction<T = unknown>(
		...
		return this.appendTypedEntry({
			...
			tokensBefore,
			details,
			// pie: crates/agent/src/harness/session/session.rs:502 — `Some(true)`/`None` only;
			// an explicit `false` must serialize the same as omitted, never as a literal `false`.
			fromHook: fromHook ? true : undefined,
	...
	async moveTo(
		...
		return this.appendTypedEntry({
			...
			// pie: crates/agent/src/harness/session/session.rs:584 — same true/undefined-only rule.
			fromHook: summary.fromHook ? true : undefined,
