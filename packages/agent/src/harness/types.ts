import type { ImageContent, Model, SimpleStreamOptions, TextContent, Transport } from "@pie/ai";
...
export interface PromptTemplate {
	...
	content: string;
	// pie: harness/types.rs:334 PromptTemplate.file_path is a required (non-Option) field.
 ...
 *
 * The values match upstream exactly. pie: `crates/agent/src/harness/types.rs:32-41`, whose enum
 * serialises in snake case, so the values on the
 * wire spell out "not a directory", "is a directory" and "invalid path" in full. This side
 * previously used shortened spellings for all three, and carried a fourth value that upstream
 * has no equivalent for and that nothing ever constructed; that one was removed.
 *
 * Upstream never constructs the two directory variants itself, but they remain values its
 * deserialiser accepts, so matching them is not merely cosmetic.
 */
 ...
 *
 * pie: `crates/agent/src/harness/types.rs:67-73` declares four values, serialised the same way:
 * a timeout, an abort, a spawn
 * failure, and unknown. One name here was corrected to match.
 *
 * Two additional values exist only on this side and are **kept deliberately** — see the
