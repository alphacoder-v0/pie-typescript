...
import { streamSimple } from "@pie/ai";
...
export interface GenerateBranchSummaryOptions {
	...
	/**
	 * Overrides the stream function; falls back to the streaming helper when absent.
	 *
	 * pie: `crates/agent/src/harness/compaction/branch_summarization.rs:26` carries the same
	 * optional and passes it down to `generate_summary`, which resolves it against the default
	 * at `compaction.rs:535`. Upstream's own tests (`compaction.rs:819/908/987`) inject a fake stream
	 * function to run without a network — the point of this seam is testability, and hard-coding
	 * the completion helper here had welded it shut.
	 */
	streamFn?: StreamFn;
}
...
export async function generateBranchSummary(
	...
	// pie: compaction.rs:535-540 — upstream resolves the optional stream function to its default,
	// then drives it.
	// The simple completion helper is itself the streaming helper followed by awaiting its
	// result, so this remains **one** path; the default has only moved to the right-hand side
	// of the nullish coalescing, and no second implementation was created.
