...
import type { AssistantMessage, Usage } from "@pie/ai";
...
describe("findCutPoint", () => {
	...
	it("should keep only the last turn when all messages fit within budget", () => {
		// pie: compaction.rs:245-269 (`find_cut_point`). Oracle starts from `target =
		// entries.len()`, so a transcript that never fills `keep_recent_tokens` cuts at the LAST
		// turn boundary and summarizes every earlier turn. This assertion previously read
		// `toBe(0)` -- the inherited pi-skeleton `cutPoints[0]` fallback, which keeps everything
		// and summarizes nothing (the opposite direction). Replaced with oracle's outcome, and
		// tightened from a single index to the whole CutPointResult.
		const entries: SessionEntry[] = [
		...
		expect(result).toEqual({ firstKeptEntryIndex: 2, turnStartIndex: -1, isSplitTurn: false });
	});
...
describe("prepareCompaction with previous compaction", () => {
	// pie: compaction.rs:245-269 + prepare_compaction (:606-623). This test used to be named
	// "should preserve kept messages across repeated compactions when they still fit" and asserted
	// the inherited pi-skeleton fallback: with everything inside `keep_recent_tokens` the cut
	// stayed at `cutPoints[0]` (= the previous compaction's own firstKeptEntryId), so u2/u3
	// survived a second compaction verbatim. Oracle instead lands on the LAST turn boundary, so
	// the earlier turns get re-summarized. Assertions re-pointed at oracle's outcome and extended
	// (the summarized set is now asserted, not just the surviving context).
	it("should re-cut at the last turn boundary across a previous compaction boundary", () => {
		...
		expect(preparation!.firstKeptEntryId).toBe(u4.id);
		...
		// The turns compaction1 had kept verbatim are now inside the summarized prefix.
		...
		expect(summarizedText).not.toContain("user msg 4 (new after compaction1)");
		...
		// Only the last turn survives verbatim; the re-summarized turns are represented by
		// compaction2's summary instead.
		expect(contextAfterText).toContain("user msg 4 (new after compaction1)");
		expect(contextAfterText).not.toContain("user msg 2 - kept by compaction1");
		expect(contextAfterText).not.toContain("user msg 3 - kept by compaction1");
	});
