...
function createArgs(messages: string[] = []): Args {
	return {
		...
		// `Args.diagnostics` is gone: every parse-time complaint is now a single clap usage error
		// (`Args.usageError`), which `main` answers with exit 2 rather than `Error: …` and exit 1.
	};
}
