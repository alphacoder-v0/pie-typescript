...
Content`,
		...
		// pie: FF4 (migration/reviews/agent/forward-flags.md) -- oracle has no AGENTS.md/CLAUDE.md
		// context-file loading step anywhere (confirmed by a codebase-wide grep of
		// crates/coding-agent/src/); RULEBOOK §4, "what this port does not do", lists this explicitly.
		// This test used
		// to assert the opposite (pi's own auto-discovery feature); flipped to assert the oracle
		// "never loads these files" behavior instead, even when they exist on disk in every
		// directory this loader would have scanned (agentDir *and* cwd).
		it("does NOT discover AGENTS.md/CLAUDE.md context files (oracle has no such loading step)", async () => {
			...
			expect(agentsFiles.some((f) => f.path.includes("AGENTS.md"))).toBe(false);
			expect(agentsFiles.some((f) => f.path.includes("CLAUDE.md"))).toBe(false);
		});

		it("still resolves to an empty agentsFiles list when noContextFiles is true (redundant with the above, kept for explicitness)", async () => {
			writeFileSync(join(cwd, "AGENTS.md"), "# Project Guidelines\n\nBe helpful.");
			writeFileSync(join(cwd, "CLAUDE.md"), "# Claude Guidelines\n\nBe helpful.");
	...
	describe("extension conflict detection", () => {
		it("should detect tool conflicts between extensions", async () => {
			...
			writeFileSync(
				...
				`
import type { ExtensionAPI } from "@pie/coding-agent";
...
// pie: FF4 (migration/reviews/agent/forward-flags.md) -- ROADMAP phase 13 acceptance criterion:
// "the system prompt contains no AGENTS.md/CLAUDE.md loading path, reconciled by grep". Static grep
// reconciliation over the source
// text itself (in addition to the behavioral test above), so a future re-introduction of the
// load path -- even one the behavioral test above doesn't happen to exercise -- is caught.
describe("FF4 grep reconciliation: no active AGENTS.md/CLAUDE.md load path", () => {
	it("loadContextFileFromDir's body contains no AGENTS.md/CLAUDE.md string outside comments", () => {
		const source = readFileSync(new URL("../src/core/resource-loader.ts", import.meta.url), "utf-8");
		const match = source.match(/function loadContextFileFromDir\([^)]*\)[^{]*\{([\s\S]*?)\n\}/);
		expect(match, "loadContextFileFromDir function body must be present to grep").not.toBeNull();
		const body = match![1]!;
		// Strip block comments (/* ... */) and line comments (// ...) before grepping, so this
		// test doesn't false-positive on the documentation comment explaining the divergence.
		const codeOnly = body
			.replace(/\/\*[\s\S]*?\*\//g, "")
			.split("\n")
			.map((line) => line.replace(/\/\/.*/, ""))
			.join("\n");
		for (const needle of ["AGENTS.md", "AGENTS.MD", "CLAUDE.md", "CLAUDE.MD"]) {
			expect(codeOnly.includes(needle), `loadContextFileFromDir must not reference ${needle} in active code`).toBe(
				false,
			);
		}
	});

	it("loadContextFileFromDir returns null unconditionally regardless of directory contents", () => {
		const dir = join(tmpdir(), `ff4-grep-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		mkdirSync(dir, { recursive: true });
		try {
			for (const filename of ["AGENTS.md", "AGENTS.MD", "CLAUDE.md", "CLAUDE.MD"]) {
				writeFileSync(join(dir, filename), "content");
			}
			const agentDir = join(tmpdir(), `ff4-grep-agent-${Date.now()}-${Math.random().toString(36).slice(2)}`);
			mkdirSync(agentDir, { recursive: true });
