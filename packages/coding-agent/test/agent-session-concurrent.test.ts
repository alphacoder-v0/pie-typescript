...
describe("AgentSession concurrent prompt guard", () => {
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		// pie: crates/agent/src/agent_loop.rs:77-80,156-158 — oracle's drive_loop checks
		// `cancel.is_cancelled()` at the top of each iteration and returns immediately; the steering
		// drain happens only *after* a successful call_llm. So an abort mid-stream ends the loop
		// without ever consuming the queued steering message. The queueing itself (asserted above)
		// is what this test is named for; consuming-after-abort was pi-only behavior.
