...
describe("test harness", () => {
	...
	it("streams thinking deltas", async () => {
		...
		const updates = harness.eventsOfType("message_update");
		// pie: crates/agent/src/agent_loop.rs:293-311 — oracle's call_llm match lists only
		// Text/Thinking/ToolCall Delta+End; the *Start variants fall into the catch-all `_ => {}`
		// and are silently dropped (no state update, no MessageUpdate event). Asserting a
		// thinking_start here would assert pi's behavior, not pie's.
	...
	it("streams tool call deltas", async () => {
		...
		const updates = harness.eventsOfType("message_update");
		// pie: crates/agent/src/agent_loop.rs:293-311 — toolcall_start is dropped by oracle's
		// catch-all, same as thinking_start above.
		...
		expect(toolcallStarts).toHaveLength(0);
	...
	it("streams thinking then text then tool call in order", async () => {
		...
		// pie: crates/agent/src/agent_loop.rs:293-311 — the *_start variants never reach the event
		// bus (oracle's catch-all drops them), so ordering is asserted on the *_delta events,
		// which oracle does emit.
		const firstThinking = streamTypes.indexOf("thinking_delta");
		const firstText = streamTypes.indexOf("text_delta");
		const firstToolcall = streamTypes.indexOf("toolcall_delta");
