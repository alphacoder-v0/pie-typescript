...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@pie/coding-agent")).toBe("Run: pnpm install -g --ignore-scripts @pie/coding-agent");
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@pie/coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@pie/coding-agent")).toBe(
			"Update @pie/coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@pie/coding-agent");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "@pie/coding-agent"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts @pie/coding-agent`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@pie/coding-agent", ["npm", "--prefix", prefix]);
	...
	test("self-update treats empty npmCommand as unset", () => {
		...
		expect(command?.args).toEqual(["--prefix", prefix, "install", "-g", "--ignore-scripts", "@pie/coding-agent"]);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(`npm --prefix "${prefix}" install -g --ignore-scripts @pie/coding-agent`);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@pie/coding-agent")).toBe("Run: npm install -g --ignore-scripts @pie/coding-agent");
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "@pie/coding-agent"],
			display: "bun install -g --ignore-scripts @pie/coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@pie/coding-agent";
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@pie/coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@pie/coding-agent")).toContain("the install path is not writable");
	});
});
