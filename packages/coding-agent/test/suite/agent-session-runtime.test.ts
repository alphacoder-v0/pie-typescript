...
import { fauxAssistantMessage, registerFauxProvider } from "@pie/ai";
...
describe("AgentSessionRuntime characterization", () => {
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		// pie: crates/agent/src/harness/session/jsonl_repo.rs:32-38 — the transcript is named
		// `<uuidv7>.jsonl`, so the stem IS the session id (jsonl_storage.rs:38-44 derives the
		// metadata id straight from `path.file_stem()`), not a timestamp-prefixed variant of it.
		const sessionFileName = parse(runtime.session.sessionFile!).name;
		expect(sessionFileName).toBe(runtime.session.sessionId);
