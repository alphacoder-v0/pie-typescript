...
// PIE_DIR is set as well because it takes precedence inside getAgentDir()
// (config.ts:485-499); setting only PI_CODING_AGENT_DIR is overridden by the PIE_DIR that test.sh
// sets from outside.
process.env.PIE_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync(process.execPath, [scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, PI_CODING_AGENT_DIR: agentDir, PIE_DIR: agentDir },
