...
describe("issue #3302 find returns no results for path-based glob patterns", () => {
	...
	async function runFind(pattern: string): Promise<string[]> {
		...
		const result = (await def.execute("call-1", { glob: pattern }, undefined, undefined, ctx)) as {
			content: Array<{ type: string; text?: string }>;
		};
		const text = result.content[0]?.text ?? "";
		// pie: crates/coding-agent/src/tools/find.rs:98-112 -- result text now starts with a
		// "find <pattern>: N hits" header line (and, when the limit was hit, a trailing
		// "... results truncated" line); skip both plus bracket-wrapped notices.
		return text
			.split("\n")
			.map((l) => l.trim())
			.filter((l) => l.length > 0 && !l.startsWith("[") && !l.startsWith("find ") && !l.startsWith("..."));
	}
