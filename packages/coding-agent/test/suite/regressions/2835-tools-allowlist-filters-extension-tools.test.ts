 ...
 * "Available tools:" heading) instead of pi's bullet list. Same discriminating power: the
 * allowlist still has to be the thing that decides what the model can see.
 */
...
describe("regression #2835: tool allowlists filter extension tools", () => {
	...
	it("allows only explicitly listed built-in and extension tools", async () => {
		...
		expect(toolInventory(session.systemPrompt)).toBe("read, dynamic_tool");
		session.dispose();
	});
