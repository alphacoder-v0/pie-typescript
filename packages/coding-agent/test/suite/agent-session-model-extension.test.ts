import type { AgentTool, ThinkingLevel } from "@pie/agent-core";
import { fauxAssistantMessage, fauxToolCall, type Model } from "@pie/ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("allows extension context handlers to modify messages before the LLM call", async () => {
		...
		if (storedUserMessage?.role === "user") {
			// A text-only prompt is stored as a bare string, matching `UserContent::Text` — oracle picks
			// the block-array arm only when images are attached (ui/kernel.rs:106-126). The
			// context-handler rewrite above is applied on the way to the provider, not to what is stored,
			// which is what this assertion pins.
			expect(storedUserMessage.content).toBe("original");
		}
	});
	...
	it("allows extension input handlers to transform or handle input", async () => {
		...
		transformedHarness.setResponses([
			(context) => {
				const user = context.messages.find((message) => message.role === "user");
				// `UserMessage.content` is `string | (TextContent | ImageContent)[]`; a text-only prompt
				// takes the string arm (oracle `UserContent::Text`). Read both so this stays a real
				// assertion instead of silently comparing against "".
				providerUserText = !user
					? ""
