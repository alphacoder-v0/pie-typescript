...
// ============================================================================
// Ported from oracle crates/coding-agent/src/templates.rs (`load_all`)
// ============================================================================

describe("loadPromptTemplates - oracle dual-root precedence (templates.rs:25,32-38)", () => {
	const roots: string[] = [];

	function makeRoots(): { cwd: string; agentDir: string; projectDir: string; globalDir: string } {
		const base = mkdtempSync(join(tmpdir(), "pie-templates-roots-"));
		roots.push(base);
		...
		return { cwd, agentDir, projectDir, globalDir };
	}

	test("project template shadows a same-name user template", () => {
		const { cwd, agentDir, projectDir, globalDir } = makeRoots();
		writeFileSync(join(globalDir, "review.md"), "---\ndescription: user copy\n---\nuser body");
		writeFileSync(join(projectDir, "review.md"), "---\ndescription: project copy\n---\nproject body");

		const templates = loadPromptTemplates({ cwd, agentDir, promptPaths: [], includeDefaults: true });

		// First match wins for every consumer (expandPromptTemplate / resource-loader dedupePrompts),
		// so the project copy must come first.
		const first = templates.find((t) => t.name === "review");
		expect(first!.description).toBe("project copy");
		expect(expandPromptTemplate("/review", templates)).toBe("project body");
	});

	test("non-colliding templates from both roots are all loaded", () => {
		const { cwd, agentDir, projectDir, globalDir } = makeRoots();
		writeFileSync(join(globalDir, "u.md"), "user only");
		writeFileSync(join(projectDir, "p.md"), "project only");

		const names = loadPromptTemplates({ cwd, agentDir, promptPaths: [], includeDefaults: true })
			.map((t) => t.name)
			.sort();
		expect(names).toEqual(["p", "u"]);
	});

	test("missing roots load silently with no diagnostics (prompt_templates.rs:80-88)", () => {
		const base = mkdtempSync(join(tmpdir(), "pie-templates-empty-"));
		roots.push(base);
		const result = loadPromptTemplatesWithDiagnostics({
			cwd: join(base, "repo"),
...
describe("loadPromptTemplates - frontmatter name (prompt_templates.rs:132-138)", () => {
	const dirs: string[] = [];

	function makeDir(): string {
		const dir = mkdtempSync(join(tmpdir(), "pie-templates-name-"));
		dirs.push(dir);
		return dir;
	}

	test("frontmatter name overrides the file stem", () => {
		const dir = makeDir();
		writeFileSync(join(dir, "01-review.md"), "---\nname: review\ndescription: d\n---\nBody");
		...
		expect(templates.map((t) => t.name)).toEqual(["review"]);
		expect(expandPromptTemplate("/review", templates)).toBe("Body");
	});

	test("file stem is used when frontmatter name is absent or empty", () => {
		const dir = makeDir();
		writeFileSync(join(dir, "stem.md"), "---\ndescription: d\n---\nBody");
		writeFileSync(join(dir, "blank.md"), '---\nname: ""\n---\nBody');

		const names = loadPromptTemplates({
			cwd: process.cwd(),
			agentDir: getAgentDir(),
			promptPaths: [dir],
			includeDefaults: false,
			...
			.map((t) => t.name)
			.sort();
		expect(names).toEqual(["blank", "stem"]);
	});
...
describe("loadPromptTemplatesWithDiagnostics - loader diagnostics (templates.rs:23,40-43)", () => {
	const dirs: string[] = [];

	function makeDir(): string {
		const dir = mkdtempSync(join(tmpdir(), "pie-templates-diag-"));
		dirs.push(dir);
		return dir;
	}

	test("malformed frontmatter yields a yaml diagnostic and skips the file", () => {
		const dir = makeDir();
		...
		// The healthy sibling still loads -- the walk continues past the failure.
		expect(result.templates.map((t) => t.name)).toEqual(["good"]);
		expect(result.diagnostics).toHaveLength(1);
		expect(result.diagnostics[0].type).toBe("warning");
		expect(result.diagnostics[0].message.startsWith("yaml: ")).toBe(true);
		expect(result.diagnostics[0].path).toBe(join(dir, "bad.md"));
	});

	test("loadPromptTemplates keeps the pi-shaped array return and drops diagnostics", () => {
		const dir = makeDir();
		writeFileSync(join(dir, "bad.md"), "---\ndescription: [unclosed\n---\nBody");
