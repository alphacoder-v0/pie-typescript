...
describe("skills", () => {
	...
	describe("collision handling", () => {
		...
		// pie: skills.rs:5,14-16,41 -- project dir is consulted AFTER user dir, and on a same-name
		// collision the project skill (loaded later) overwrites the user one ("project can
		// override a user-wide skill of the same name" / `dedupe_project_wins`). This is the
		// includeDefaults:true path used by `loadEffectiveSkills()` (tools/skill.ts), the closest
		// analog to oracle's live skill catalog -- see the RULEBOOK user-dir-layout.md arbitration.
		describe("default project vs user collision (oracle: project wins)", () => {
			let agentDir: string;
			let cwd: string;

			beforeEach(() => {
				agentDir = mkdtempSync(join(tmpdir(), "skills-collision-agent-"));
				cwd = mkdtempSync(join(tmpdir(), "skills-collision-cwd-"));
			});
			...
			function writeUserSkill(name: string, description: string): string {
			...
			function writeProjectSkill(name: string, description: string): string {
				const dir = join(cwd, ".pie", "skills", name);
				mkdirSync(dir, { recursive: true });
				const filePath = join(dir, "SKILL.md");
				writeFileSync(filePath, `---\nname: ${name}\ndescription: ${description}\n---\nProject body\n`);
				return filePath;
			...
			it("project skill wins over a same-name user skill", () => {
				writeUserSkill("shared-skill", "User description");
				const projectPath = writeProjectSkill("shared-skill", "Project description");
				...
				expect(skills).toHaveLength(1);
				expect(skills[0].filePath).toBe(projectPath);
				expect(skills[0].description).toBe("Project description");

				const collision = diagnostics.find(
					(d): d is ResourceDiagnostic & { collision: NonNullable<ResourceDiagnostic["collision"]> } =>
						d.type === "collision" && d.collision?.name === "shared-skill",
				);
				expect(collision).toBeDefined();
				expect(collision?.collision.winnerPath).toBe(projectPath);
				expect(collision?.collision.loserPath).toContain(join(agentDir, "skills", "shared-skill"));
			});
		});
	});
});
