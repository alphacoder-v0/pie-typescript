...
describe("migrateSessionEntries", () => {
	it("should add id/parentId to v1 entries", () => {
		...
		// Backfilled ids are full uuidv7s, matching what `create_entry_id` hands out on the oracle
		// side (jsonl_storage.rs:178-180 `Ok(uuidv7())`). pi used `randomUUID().slice(0, 8)` here.
		const UUIDV7 = /^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;

		expect(msg1.id).toBeDefined();
		expect(msg1.id).toMatch(UUIDV7);
		expect(msg1.parentId).toBeNull();

		expect(msg2.id).toBeDefined();
		expect(msg2.id).toMatch(UUIDV7);
