...
/**
 * A transcript's header line, as written to disk.
 *
 * pie: session.rs:194-219 (`SessionMetadata` + `JsonlSessionMetadata`) — `{id, createdAt, cwd,
 * path, ...}`. Unlike every entry line it has no `type` discriminator (position identifies it),
 * so "is this the header?" is a shape test, not a tag test.
 */
function isHeaderRecord(record: Record<string, unknown> | undefined): boolean {
	return (
		!!record &&
...
describe("createBranchedSession", () => {
	...
	it("does not duplicate entries when forking from first user message", () => {
		...
		try {
			...
			// The header carries no `type` tag on disk (oracle `JsonlSessionMetadata`, session.rs:194-219),
			// so it is identified the way oracle identifies it: by being the metadata record.
			expect(records.filter(isHeaderRecord)).toHaveLength(1);
			expect(isHeaderRecord(records[0])).toBe(true);

			const entryIds = records
				.filter((r) => !isHeaderRecord(r))
				.map((r) => r.id)
