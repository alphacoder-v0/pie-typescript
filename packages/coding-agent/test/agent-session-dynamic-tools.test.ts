 ...
 * (main.rs:1203-1225 `render_base_prompt` — a comma-joined name list built from the live
 * registry, with no per-tool blurbs and no guideline list).
 */
...
describe("AgentSession dynamic tool registration", () => {
	...
	it("refreshes tool registry when tools are registered after initialization", async () => {
		...
		// pie: main.rs:1203-1225 — a late-registered tool joins the inventory by name. Oracle has no
		// per-tool `promptSnippet` or `promptGuidelines` concept, so neither string reaches the prompt.
		expect(toolInventory(session.systemPrompt).split(", ")).toContain("dynamic_tool");
	...
	it("lists snippet-less custom tools in the inventory (oracle lists every registered tool)", async () => {
		const settingsManager = SettingsManager.create(tempDir, agentDir);
		const sessionManager = SessionManager.inMemory();
		...
		// pie: main.rs:669-676 + 1203-1207 — `tool_names` is every registered tool; oracle has no
		// notion of a tool that is callable but hidden from the prompt. Only the description stays
		// out, because the inventory is names only.
		expect(toolInventory(session.systemPrompt).split(", ")).toContain("hidden_tool");
