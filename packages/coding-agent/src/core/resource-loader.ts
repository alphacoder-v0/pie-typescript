...
import { loadSkills, mergeSkillsWithBuiltins } from "./skills.ts";
...
export interface ResourceLoader {
	...
	/**
	 * Re-merge the skill catalog from the already-resolved paths. Cheap next to `reload()`:
	 * no settings reload, no package-manager resolve, no extension re-resolution.
	 *
	 * Exists because oracle resolves `--builtin-skill` / `[builtin_skills] enabled` AFTER the
	 * on-disk catalog is loaded and folds them in at harness-construction time
	 * (main.rs:695-707). This side builds the session first, so the resolution point has to be
	 * able to say "now re-merge" without re-doing the whole discovery pass.
	 */
	refreshSkills(): void;
	reload(): Promise<void>;
}
...
/**
 * pie: FF4 (migration/reviews/agent/forward-flags.md) -- audited codebase-wide (`grep -rni
 * "AGENTS.md\|CLAUDE.md" crates/coding-agent/src/`): the oracle has zero references to
 * AGENTS.md/CLAUDE.md anywhere in its context-loading path. This is a confirmed, deliberate
 * divergence from pi, not an oversight -- RULEBOOK §4 "what this port does not do" lists it explicitly: "do not
 * load AGENTS.md/CLAUDE.md into the system prompt (upstream behavior)". `loadProjectContextFiles` below therefore always
 * returns an empty list; kept as a no-op function (rather than deleted outright) so
 * `loadProjectContextFiles`/`DefaultResourceLoader.getAgentsFiles()`'s public shape is
 * unaffected for callers -- see resource-loader.test.ts's grep-reconciliation test asserting
 * this behavior end-to-end even when AGENTS.md/CLAUDE.md exist on disk.
 */
function loadContextFileFromDir(_dir: string): { path: string; content: string } | null {
	return null;
}
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	private updateSkillsFromPaths(skillPaths: string[], metadataByPath?: Map<string, PathMetadata>): void {
		...
		// pie: main.rs:698-707 folds the resolved built-in catalog in with `merge_with_user_project`
		// (builtin_skills.rs:196-206) — built-ins first, a user/project skill of the SAME NAME
		// replaces the built-in in place. `enabledBuiltinSkills()` is empty unless `--builtin-skill`
		// or `[builtin_skills] enabled` asked for something, so this is a no-op by default.
		//
		// This is the loader every skill consumer actually reads (`/skills`, the startup line, the
		// system-prompt catalog). phase 21 batch A's lesson applies directly: publishing into a
		// registry only helps if the real consumer reads it — the first cut of this wiring merged
		// into `tools/skill.ts`'s `loadEffectiveSkills`, which the running CLI never calls for the
		// catalog, so `--builtin-skill` stayed silently inert. The end-to-end check caught it.
		const withBuiltins = mergeSkillsWithBuiltins(skillsResult);
		const resolvedSkills = this.skillsOverride ? this.skillsOverride(withBuiltins) : withBuiltins;
	...
	private updatePromptsFromPaths(promptPaths: string[], metadataByPath?: Map<string, PathMetadata>): void {
		...
		if (this.noPromptTemplates && promptPaths.length === 0) {
			...
			// pie: main.rs:1001-1006 — oracle keeps `loaded_templates.diagnostics` from
			// `templates::load_all` and reports them at startup. The plain `loadPromptTemplates`
			// wrapper throws them away, so this call site takes the diagnostics-carrying variant and
			// concatenates the loader's findings ahead of the dedupe/conflict ones (loader-order
			// first, matching oracle, whose reported "first" diagnostic is a load diagnostic).
	...
	private getDefaultSourceInfoForPath(filePath: string): SourceInfo {
		// Two synthetic shapes reach here, and both must stay off the filesystem — the tail of this
		// method ends in `statSync`, which throws ENOENT on anything that isn't really there.
		//
		//   `<temporary>`                    whole path is the marker (the original case)
		//   `<builtin>/<name>/SKILL.md`      marker is only the FIRST SEGMENT
		//
		// The second is oracle's own format for built-in skills (`builtin_skills.rs:141-152`:
		// `format!("<builtin>/{}/SKILL.md", spec.name)`), so it is a path that never exists by
		// construction. `findSourceInfoForPath` already routes every `<`-prefixed path straight
		// here, so this guard is the one place that decides "synthetic or not".
		const syntheticMarker = filePath.startsWith("<")
			? filePath.endsWith(">")
				? filePath
				: /^<[^>]*>(?=[/\\])/.exec(filePath)?.[0]
			: undefined;
		if (syntheticMarker) {
			return {
				path: filePath,
				source: syntheticMarker.slice(1, -1).split(":")[0] || "temporary",
				scope: "temporary",
				origin: "top-level",
			};
		}
