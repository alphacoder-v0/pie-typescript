...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@pie/coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@pie/coding-agent";
	 *
...
export interface ToolDefinition<TParams extends TSchema = TSchema, TDetails = unknown, TState = any> {
	...
	/**
	 * Per-tool permission classification, evaluated by the agent loop against the prepared args
	 * *before* `beforeToolCall` runs (`packages/agent/src/agent-loop.ts:655-680`). Return
	 * `{ type: "prompt", reason }` to route the call through
	 * `AgentLoopConfig.onControlPlanePrompt`, or `{ type: "block", reason }` to hard-deny.
	 * Omitted behaves as `{ type: "allow" }`.
	 *
	 * Mirrors `AgentTool.permissionClassification` (`packages/agent/src/types.ts:476-484`). It
	 * lives on the definition as well because AgentSession's registry is definition-first: every
	 * registered tool is round-tripped through `wrapToolDefinition()`, so a classifier attached
	 * only to the `AgentTool` would be silently dropped before it ever reached the loop.
	 * pie: crates/coding-agent/src/tools/{install_skill,skill_builder,set_skill_state,remove_skill}.rs
	 * (`permission_classification`), which oracle reads straight off the registered
	 * `Arc<dyn AgentTool>` (no definition/tool split exists there).
	 *
	 * Declared with METHOD syntax, not `?: (args) => …`, deliberately. Under `strictFunctionTypes`
	 * a property holding a function type is checked contravariantly in its parameters, so a
	 * concrete `ToolDefinition<TObject<{path,…}>>` would stop being assignable to the erased
	 * `ToolDefinition<any, any, any>` that every registry/consumer signature uses — `Static<any>`
	 * widens to `unknown`, and `(args: {path: string}) => …` is not assignable to
	 * `(args: unknown) => …`. Method syntax keeps bivariant parameter checking, which is exactly
	 * why `execute()` above is declared the same way.
	 */
	permissionClassification?(preparedArgs: Static<TParams>): PermissionClassification;
