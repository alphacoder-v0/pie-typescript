...
import { type Api, type KnownProvider, type Model, modelsAreEqual } from "@pie/ai";
...
/**
 * Where to look when `--provider`/`--model` names something the catalog does not have.
 *
 * Phase 19 F8: this used to read "Use --list-models to see available models", and `--list-models`
 * appears **nowhere in `pie --help`** — it is a pi flag oracle never grew, and oracle answers it
 * with `error: unexpected argument '--list-models' found … tip: a similar argument exists:
 * '--list-sessions'`. So the one next step the message offered was one the user could not have
 * found and could not have read about.
 *
 * `/model list` is the route `pie --help` itself documents, byte-identically on both sides, in the
 * `Model catalog:` block's third line ("Full list: /help models or /model list [provider]"). It
 * leads, and `--list-models` follows as the non-interactive spelling that does work here — naming
 * it in the error is now the *second* pointer rather than the only one.
 *
 * What this deliberately does NOT do is adopt oracle's behavior for the surrounding branch. Oracle
 * ignores an unknown `--model` outright and silently starts on the auto-detected default (measured:
 * `--model no-such-model-xyz` → exit 0 on Haiku). That is the same failure shape phase 19's F4 just
 * removed — the user is put on a model they did not ask for, with no notice — so the hard-fail
 * stays. It is therefore an INTENTIONAL DIVERGENCE and needs a row in
 * `migration/parity/intentional-divergences.md`; the declaration text ships with the F8 hand-off
 * rather than being written here, because that ledger is not this unit's to edit. No parity
 * scenario drives an unknown `--model`, so the judged baseline is unaffected either way.
 */
const MODEL_DISCOVERY_HINT =
...
export function resolveCliModel(options: {
	...
	if (cliProvider && !provider) {
		return {
			model: undefined,
			warning: undefined,
			error: `Unknown provider "${cliProvider}". ${MODEL_DISCOVERY_HINT}`,
		};
	}
	...
	return {
		...
		thinkingLevel: undefined,
		warning,
		error: `Model "${display}" not found. ${MODEL_DISCOVERY_HINT}`,
	};
}
