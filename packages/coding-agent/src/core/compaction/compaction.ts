...
import type { AgentMessage, StreamFn, ThinkingLevel } from "@pie/agent-core";
import type { AssistantMessage, Context, Model, SimpleStreamOptions, Usage } from "@pie/ai";
import { completeSimple } from "@pie/ai";
import {
	convertToLlm,
...
export async function compact(
	...
	// summarizer. The caller then drops the empty summary before persisting a compaction entry
	// or emitting any event (agent_harness.rs:2088-2090 + 2098-2099). Without this guard a
	// transcript whose entire history is already inside the keep-recent window still round-trips
	// through the summarizer and produces a bogus "compaction" of nothing.
	if (messagesToSummarize.length === 0 && turnPrefixMessages.length === 0) {
		const empty = computeFileLists(fileOps);
		return {
			summary: "",
			firstKeptEntryId,
			tokensBefore,
			details: { readFiles: empty.readFiles, modifiedFiles: empty.modifiedFiles } as CompactionDetails,
		};
	}
