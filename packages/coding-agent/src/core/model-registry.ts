...
import { registerOAuthProvider, resetOAuthProviders } from "@pie/ai/oauth";
...
export class ModelRegistry {
	...
	/**
	 * Snapshot of oracle's process-global custom-model registry (`pie_ai::register_custom_model`,
	 * crates/ai/src/models.rs:41-50), populated by the CLI's `local-models.ts` before model
	 * resolution. Held as its own list — not folded into {@link models} — so it survives
	 * {@link refresh}, exactly as the oracle global survives a models.json reload.
	 * See the scope note at the top of `../local-models.ts`.
	 */
	private localModels: Model<Api>[] = [];
	...
	private loadModels(): void {
		...
		// pie: crates/ai/src/models.rs:23-30 (`get_model`) consults the custom registry BEFORE the
		// static catalog, so a local model always wins over a built-in with the same provider/id.
		// Re-applied on every (re)load because oracle's registry is a process global that a
		// models.json reload cannot clear.
		this.models = this.localModels.length > 0 ? this.mergeCustomModels(combined, this.localModels) : combined;
	}

	/**
	 * Publish the CLI's local/custom model registry (oracle `local_models::load_all`, which registers
	 * into the `pie_ai` process global) into this registry, which is the TS stand-in for the merged
	 * `pie_ai::get_model` / `list_models` view every consumer reads.
	 *
	 * pie: crates/coding-agent/src/main.rs:551 — `local_models::load_all(&cwd, …)` runs immediately
	 * before `model::auto_detect_model`, so callers must invoke this before model resolution.
	 */
	setLocalModels(models: readonly Model<Api>[]): void {
		this.localModels = [...models];
		if (this.localModels.length > 0) {
			// Merge in place rather than via `refresh()`: a reload would reset the dynamic API/OAuth
			// registrations that extensions may already have installed, which registering a local
			// model has no business doing. `loadModels()` re-applies `localModels` on its own.
			this.models = this.mergeCustomModels(this.models, this.localModels);
		}
	}
