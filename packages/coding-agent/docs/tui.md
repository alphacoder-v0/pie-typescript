...
**Source:** [`@pie/tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@pie/tui";
...
```typescript
import { Container, type Focusable, Input } from "@pie/tui";
...
```typescript
import { Text, Box, Container, Spacer, Markdown } from "@pie/tui";
```
...
```typescript
import { matchesKey, Key } from "@pie/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@pie/tui";
...
```typescript
import { getMarkdownTheme } from "@pie/coding-agent";
import { Markdown } from "@pie/tui";
...
For letting users pick from a list of options. Use `SelectList` from `@pie/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@pie/coding-agent";
import { DynamicBorder } from "@pie/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@pie/tui";
...
```typescript
import { BorderedLoader } from "@pie/coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@pie/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@pie/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@pie/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@pie/coding-agent";
import { matchesKey, truncateToWidth } from "@pie/tui";
