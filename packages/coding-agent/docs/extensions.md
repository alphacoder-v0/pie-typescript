...
| Package | Purpose |
|---------|---------|
| `@pie/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@pie/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@pie/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@pie/coding-agent";
...
```typescript
import { isBashToolResult } from "@pie/coding-agent";
...
```typescript
import { createLocalBashOperations } from "@pie/coding-agent";
...
```typescript
import { SessionManager } from "@pie/coding-agent";
...
```typescript
import type { AutocompleteItem } from "@pie/tui";
...
```typescript
import { withFileMutationQueue } from "@pie/coding-agent";
...
**Important:** Use `StringEnum` from `@pie/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@pie/coding-agent";
...
```typescript
import { createBashTool } from "@pie/coding-agent";
...
```typescript
import { keyHint } from "@pie/coding-agent";
...
```typescript
import { Text, Component } from "@pie/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@pie/coding-agent";
