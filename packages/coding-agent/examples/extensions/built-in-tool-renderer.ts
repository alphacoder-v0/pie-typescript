...
import type { BashToolDetails, EditToolDetails, ExtensionAPI, ReadToolDetails } from "@pie/coding-agent";
import { createBashTool, createEditTool, createReadTool, createWriteTool } from "@pie/coding-agent";
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme, _context) {
			...
			// pie: read.rs:78-84 — `details` is oracle's {path, totalLines, keptLines, offset}; a read
			// dropped lines whenever it kept fewer than the scan walked.
			if (details && details.keptLines < details.totalLines) {
				text += theme.fg("warning", ` (truncated from ${details.totalLines})`);
			}
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme, _context) {
			...
			// BashToolDetails tracks stdout/stderr truncation separately (no combined `truncation`
			// field); show the badge if either stream was truncated.
			if (details?.stdoutTruncation?.truncated || details?.stderrTruncation?.truncated) {
